...
Assume true underlying fatitilty rate is equal $\mu$. (Potential for hierarcical model).
Only look at data age >=60.

missreporting rate = $p \in (0,1)$

probability observing d death on ship is poison 
...
Point estimate of true mortality

```{r}
mu_p(0.48)
```

Lower and upper bounds of true mortality
```{r}
mu_p(0.18)
mu_p(0.79)
```

The figure of 150,123 true cases is for the date of February 26th. 
This [source](https://www.who.int/docs/default-source/coronaviruse/situation-reports/20200226-sitrep-37-covid-19.pdf) says there was 78191 confirmed cases in China at that date. To get the true number of cases, you divide by the 1-misreporting rate. 
```{r}
78191/(1-0.48)
```
Not exactly 150 123 but close. 

In the case of Japan there was 167 cases, and the article says 321 trues cases
```{r}
167/(1-0.48)
```

As for true mortality in China: the article says 1.19
