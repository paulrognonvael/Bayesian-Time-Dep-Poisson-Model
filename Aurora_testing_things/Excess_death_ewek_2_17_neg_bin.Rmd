...
```{r}
alpha <- 8.1503 #shape
beta <- 0.0028544 #1/scale

# =>

p = beta/(1+beta) # as a mean
```


a/(a+b) = 0.00285/(1+0.00285) , a*b/((a+b)^2*(a+b+1)) = 0.05^2
...
```{r}
death_netherland <- stan("exp_death_week_2_17_neg_bin.stan", iter = 2000, chains = 4,
...
```{r}
n_sim <- 1000
post_predict1 <- rnbinom(n_sim, size = posterior[, "r1"], prob=(posterior[, "p1"]))
post_predict2 <- rnbinom(n_sim, size = posterior[, "r2"], prob=(posterior[, "p2"]))
post_predict3 <- rnbinom(n_sim, size = posterior[, "r3"], prob=(posterior[, "p3"]))
post_predict4 <- rnbinom(n_sim, size = posterior[, "r4"], prob=(posterior[, "p4"]))
