...
confint(model.1)
#Confidence interval for RR que son exponencial(beta)
data.frame(RR=exp(model.1$coefficients),Lower=exp(confint(model.1))[,1],Upper=exp(confint(model.1))[,2])
#CUIDADO si hay un en el exp(coeff)=>no significativo 


summary(model.1)
#CIUDADO ahi avisa que la dispersion tomado como 1 !!

#SOBREDISPERSION: resifual deviance/grados de libertad (dados en summary)
...
summary(model.1q)
#para tomar en cuenta la sobre-dispersion sin modelizarla, sino subestima los errores. 

#correlacion positiva genera sobre-dispersion
#correlacion negativo genera infra-dispersion pero no es probable en epidemiologia


#Negative binomial regression permite recoger la sobre dispersion, otro método 
...
summary(model.1nb)
####CIUDADO EL MODELO NO AJUSTA


#Podemos usar AIC para elegir entre negative binomila y poisson
