...
#covid_data
data_covid <- read.csv("france_belgium_norway.csv",)
data_covid_england <- read.csv("england_wales_weekly_covid_deaths.csv")
...
```{r, include= FALSE}
data_list_covid_netherland <- create_list( length(mean_netherland),mean_netherland,netherland_2020 )
...
data_france <- create_CI_theta_vec(risk_france,16)
plot_risk(data_france, 'france')


data_norway <- create_CI_theta_vec(risk_norway, 17)
plot_risk(data_norway, 'norway')

data_belgium <- create_CI_theta_vec(risk_belgium, 17)
plot_risk(data_belgium, 'belgium')

data_england <- create_CI_theta_vec(risk_england_wales, 17)
plot_risk(data_england, 'england')
```
