...
Our data is characterized by much larger mean than variance. For example, Table \ref{tab:tab1} shows the weekly means and variance for Belgium.

```{r, eval = TRUE}
trans_belgium <- t(data_belgium)[2:ncol(data_belgium),]
...
kable(df, "latex",
  ...
  caption = "\\label{tab:tab1} Means and variances of weekly deaths in France"
  ) %>%
  kable_styling(
...
where:
$$\theta_{t,i} = \alpha+\beta\theta_{t,i-1}$$
...
To model excess mortality on top of reported COVID deaths, we use the same approach as for modelling of excess mortality during the COVID-19 outbreak with respect to non-outbreak times. We estimated the relative risk $\theta$ understood as the ratio of weekly mortality during the COVID-19 outbreak and the weekly expected mortality in non-outbreak times. We now adjust the exptected mortality $E_i$ to account for COVID deaths.
...
* complete the EDA with missing countries
